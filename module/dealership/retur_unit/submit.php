...
foreach($data as $dt){
	$sql = "insert into repayment_rv(rrv_spuk_id,rrv_no_rv) values('$spuk_id','$dt->rv_no')";
...
$sql3 = "update spuk_dtl set spuk_dtl_status='R' where spuk_dtl_id='$spuk_id'";
...
if($hasil3 == false){
	...
	while($data3 = mysqli_fetch_array($hasil3)){
		...
		$sql4 = "insert into wf_history(wf_hist_id,wf_hist_seq,no_proses,wf_hist_date_create,outlet_proses,wf_hist_dept,wf_hist_job) values('$wf_id','$wf_hist_seq','$spuk_id',now(),'$_SESSION[outlet]','$wf_dtl_dept','$wf_dtl_job')";

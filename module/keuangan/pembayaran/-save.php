...
/* $sql15 = "update unit_titip_jual set
		...
		utj_updated=now()
		where utj_id in (select spuk_dtl_utj from spuk_dtl where spuk_dtl_id='$pv_proses_id')";
...
$sql3 = "update fin_trn_rv set
		rv_status='C'
		where rv_no in (select rrv_no_rv from repayment_rv where rrv_spuk_id='$pv_proses_id')";
$result3 = mysqli_query($con,$sql3);
if($result3 == false){
	array_push($error,mysqli_error($con));
} */

$query7 = "select spuk_dtl_utj from spuk_dtl where spuk_dtl_id='$pv_proses_id'";
...
$query8 = "select rrv_no_rv from repayment_rv where rrv_spuk_id='$pv_proses_id'";
...
$query3 = "select 
		...
		pv_rv_amount,
		spuk_outlet,
		(select margin_amount from mst_margin where margin_id='M2') as titipan_operasional
	from spuk_hdr a
...
if($hasil3 == false){
	...
	$spuk_jml_unit = $data3['spuk_jml_unit'];
	$pv_amount = $data3['pv_amount'];
	$pv_scheme = $data3['pv_scheme'];
	$pv_rv_amount = $data3['pv_rv_amount'];
	$spuk_outlet = $data3['spuk_outlet'];
	$titipan_operasional = $data3['titipan_operasional']*$spuk_jml_unit;
	$administrasi_mokas = ($pv_scheme-$titipan_operasional)/1.1;
	$hutang_ppn = $administrasi_mokas*0.1;
	$margin = $pv_scheme-$titipan_operasional;
}

//jurnal Pelunasan Unit ke FIF
$sql4 = "insert into gl_journal(gl_no,gl_period_month,gl_period_year,gl_date,gl_type,gl_desc,gl_segment1,gl_segment2,gl_dr,gl_create_by,gl_created) values('$pv_no','$month','$year',now(),'OUT','$pv_desc','$_SESSION[outlet]','3310201','$pv_amount','$_SESSION[uid]',now())";
$result4 = mysqli_query($con,$sql4);
if($result4 == false){
	array_push($error,mysqli_error($con));
}

$sql5 = "insert into gl_journal(gl_no,gl_period_month,gl_period_year,gl_date,gl_type,gl_desc,gl_segment1,gl_segment2,gl_cr,gl_create_by,gl_created) values('$pv_no','$month','$year',now(),'OUT','$pv_desc','$_SESSION[outlet]','$rekout_segment','$pv_amount','$_SESSION[uid]',now())";
...
$sql6 = "insert into gl_journal(gl_no,gl_period_month,gl_period_year,gl_date,gl_type,gl_desc,gl_segment1,gl_segment2,gl_dr,gl_create_by,gl_created) values('$jv_no','$month','$year',now(),'JV','$pv_desc','$_SESSION[outlet]','3310201','$pv_scheme','$_SESSION[uid]',now())";
$result6 = mysqli_query($con,$sql6);
if($result6 == false){
	array_push($error,mysqli_error($con));
}

$sql7 = "insert into gl_journal(gl_no,gl_period_month,gl_period_year,gl_date,gl_type,gl_desc,gl_segment1,gl_segment2,gl_cr,gl_create_by,gl_created) values('$jv_no','$month','$year',now(),'JV','$pv_desc','$spuk_outlet','3310101','$pv_scheme','$_SESSION[uid]',now())";
...
$query4 = "select 
	...
	where rrv_spuk_id='$pv_proses_id'
...
if($hasil4 == false){
	...
	while($data4 = mysqli_fetch_array($hasil4)){
		...
		$query5 = "select pv_calculate from fin_trn_payment where pv_proses_id='$pv_proses_id'";
		$hasil5 = mysqli_query($con,$query5);
		if($hasil5 == false){
			array_push($error,mysqli_error($con));
		} else {
		...
		if($pv_calculate > $rv_amount){
			$sql10 = "update fin_trn_rv set rv_used=rv_used+'$rv_amount',rv_amount=rv_amount-rv_used where rv_no='$rrv_no_rv'";
			$result10 = mysqli_query($con,$sql10);
			if($result10 == false){
				array_push($error,mysqli_error($con));
			}
			
			$sql11 = "update fin_trn_payment set pv_calculate='$selisih' where pv_proses_id='$pv_proses_id'";
		...
		if($pv_calculate < $rv_amount){
			$sql12 = "update fin_trn_rv set rv_used=rv_used+'$pv_calculate',rv_scheme=rv_scheme+'$pv_scheme',rv_amount=rv_amount-'$pv_calculate'-'$pv_scheme' where rv_no='$rrv_no_rv'";
			$result12 = mysqli_query($con,$sql12);
			if($result12 == false){
				array_push($error,mysqli_error($con));
			}
			
			$sql13 = "update fin_trn_payment set pv_calculate=0 where pv_proses_id='$pv_proses_id'";
			...
			if($hasil6 == false){
				...
				/* if($sisa_rv > $scheme){
					$sql14 = "update fin_trn_rv set rv_status='N',rv_classification=NULL where rv_no='$rrv_no_rv'";

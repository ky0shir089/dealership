...
$supl_id = htmlspecialchars(strtoupper($_REQUEST['supl_id']));
$rek_no = htmlspecialchars(strtoupper($_REQUEST['rek_no']));
...
$sql = "insert into fin_trn_inv_hdr(invhdr_no,invhdr_supplier,invhdr_rek_no,invhdr_desc,invhdr_amount,invhdr_segment1,invhdr_segment2,invhdr_mst_code,invhdr_create_by,invhdr_created) values('$inv_no','$supl_id','$rek_no','$invhdr_desc','$invhdr_amount','$_SESSION[outlet]','$invhdr_segment2','$invhdr_mst_code','$_SESSION[uid]',now())";
...
foreach($data as $dt){
	$sql4 = "insert into used_rv_promo(used_invoice_no,used_rv_no,used_rv_amount) values('$inv_no','$dt->rv_no','$dt->rv_scheme')";
	$result4 = mysqli_query($con,$sql4);
	if($result4 == false){
		array_push($error,mysqli_error($con));
	}

	$sql3 = "update fin_trn_rv set rv_status='U' where rv_no='$dt->rv_no'";
